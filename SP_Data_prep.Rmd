...
sites <- read.csv(url("https://knb.ecoinformatics.org/knb/d1/mn/v2/object/urn%3Auuid%3A577e1c7f-7a8d-42ca-bb5d-e0de70c90480", method = "libcurl"), header = TRUE, stringsAsFactors = FALSE)
...
## POW water ##
h2o.utm <- readOGR(dsn = "E:/wraymond2/My Documents/Graduate School/APECS_master/ALL_DATA/spatial_data", layer = "POW_water_UTM")

h2o.utm <- fix_geo_problems(h2o.utm, tries = 5)

h2o.latlong <- spTransform(h2o.utm, CRS("+init=epsg:4326"))
h2o.latlong <- fix_geo_problems(h2o.latlong, tries = 5)
```
...
otts.sp <- SpatialPointsDataFrame(coords = otts.xy, data = otts, proj4string = CRS("+proj=utm +zone=8 +ellps=GRS80 +units=m +no_defs +datum=WGS84"))
...
sites.sp <- SpatialPointsDataFrame(coords = sites.xy, data = sites, proj4string = CRS("+proj=utm +zone=8 +ellps=GRS80 +units=m +no_defs +datum=WGS84"))
```
...
for(i in unique(1:26)){
  ...
  bw <- gIntersection(h2o.utm, b, byid = TRUE) # clip the buffer area by water so we only have water area left
  ...
  o <- over(p, otts.sp) # identifies surveyed otters that intersect/overlap with the polygon
