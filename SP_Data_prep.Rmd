...
sites <- unique(pops[, 1:3])

xy <- sites[, c(2,3)]

sites <- SpatialPointsDataFrame(coords = xy, data = sites, proj4string = CRS("+proj=utm +zone=8 +ellps=GRS80 +towgs84=0,0,0 +datum=WGS84"))

## Calculate survey area ##
buff <- gBuffer(sites[1, ], byid = TRUE, width = 3704, quadsegs = 10) # create buffer
